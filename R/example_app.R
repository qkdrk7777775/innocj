...
example_app <- function(app_dir = "app", wd = getwd(), type = "Shiny") {
  ...
  if (grepl("Shiny", type, TRUE)) {
    ...
    app_files <- list.files(system.file("app", package = "innocj"), full.names = TRUE, pattern = ".R")
    data_files <- list.files(system.file("app/data", package = "innocj"), full.names = TRUE)
    www_files <- list.files(system.file("app/www", package = "innocj"), full.names = TRUE)
    html_files <- list.files(system.file("app/html", package = "innocj"), full.names = TRUE)
    local_files <- list.files(system.file("app/local", package = "innocj"), full.names = TRUE)
    ...
    x <- list.files(
      system.file("app/flexdashboard_example", package = "innocj"),full.names = T)
